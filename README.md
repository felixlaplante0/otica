...
For observations generated by the linear ICA model $X = S A^\top$, **otica** first centers and whitens the data. For whitened observations $Z \in \mathbb{R}^{n \times d}$, it finds an orthogonal unmixing matrix $W \in \mathbb{R}^{d \times d}$ that maximizes
